...
test('Scatter: X readies it, X again fires five big red shells; 40 s cooldown; no shells used',()=>{
 ...
 s.step({scatter:true});assert.equal(s.scatterShells.length,0,'not before it has charged for 3 s');assert.equal(s.scatter.armed,true);
 ticks(s,Math.ceil(SCATTER.prime*60));assert.ok(s.events.some(e=>e.type==='scatterPrimed'),'charged');
...
test('each big shell splits into four that each end in a little explosion',()=>{
 const s=make();s.step({scatter:true});s.scatter.armedFor=SCATTER.prime;s.step({scatter:true});ticks(s,90);
...
test('damage: moderate from big shells, decent from the small ones, stacking splash, capped',()=>{
 ...
 const near=make(),t=dummy(1.3);near.targets.push(t);near.step({scatter:true});near.scatter.armedFor=SCATTER.prime;near.step({scatter:true});ticks(near,90);
 ...
 const mid=make(),m=dummy(4.5);mid.targets.push(m);mid.step({scatter:true});mid.scatter.armedFor=SCATTER.prime;mid.step({scatter:true});ticks(mid,90);
 assert.ok(5000-m.hp>=120&&5000-m.hp<=250,'mid-range '+(5000-m.hp));
 // Where the small shells come down: several small hits and bursts.
 const far=make(),f=dummy(8.5);far.targets.push(f);far.step({scatter:true});far.scatter.armedFor=SCATTER.prime;far.step({scatter:true});ticks(far,90);
...
test('walls shelter from the little explosions, and a readied Scatter is dropped when you come back',()=>{
 ...
 s.step({scatter:true});s.scatter.armedFor=SCATTER.prime;s.step({scatter:true});ticks(s,90);assert.equal(t.hp,5000);
 const d=make();d.step({scatter:true});d.respawn({x:0,z:0});assert.equal(d.scatter.armed,false,'a respawn drops it');
});
test('Scatter bursts set crops alight',()=>{
 const s=make();s.crops=[{id:'c',x:8.5,z:0,w:6,d:6,state:'standing'}];
 s.step({scatter:true});s.scatter.armedFor=SCATTER.prime;s.step({scatter:true});ticks(s,90);
 assert.equal(s.crops[0].state!=='standing',true,'the field caught: '+s.crops[0].state);
});
