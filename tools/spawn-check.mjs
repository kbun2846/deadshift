...
//    screen (SPAWN_APART) away, and pairs the ground hides from each other
//    (ground.sightClear), within 40 m;
//  - base sightlines: from every base point, rays every 4 degrees out to
//    where something stops sight (the ground, a sight-blocking collider) or
...
const ground = groundFor(map), colliders = mapColliders(map), blockers = colliders.filter(c => c.blocksSight);
...
console.log(`${map.name}: ${map.bases?.length || 0} bases, ${map.ffaSpawns?.length || 0} FFA points, ${colliders.length} colliders (${blockers.length} block sight)\n`);
...
for (const p of pts) {
 ...
 const close = others.filter(q => d(p, q) < 40), hidden = close.filter(q => !ground.sightClear(p.x, p.z, q.x, q.z)).length;
 minPair = Math.min(minPair, nearest); nearPairs += close.length; hiddenPairs += hidden;
 console.log('  ' + row('ffa', p) + `  nearest ${f1(nearest)} m, ${apart}/${others.length} a screen away, ground hides ${hidden}/${close.length} within 40 m`);
}
console.log(`  closest pair ${f1(minPair)} m; the ground hides ${hiddenPairs / 2}/${nearPairs / 2} pairs within 40 m`);
