...
// Editing needs the controls on screen, so it closes the menus and starts from
// the running match; outside a match the button explains that instead.
const byId = id => document.getElementById(id);

// state() -> { started, paused, deathActive, touchPrompts }
export function installMobileSettings({ touchLayout, closeSettings, setPaused, state, arrange }) {
 function sync() {
  const { started, deathActive, touchPrompts } = state();
  arrange();
  byId('touch-swap-sides').setAttribute('aria-pressed', String(touchLayout.swapped));
  byId('touch-edit-layout').disabled = !(started && !deathActive && touchPrompts);
  byId('touch-edit-note').textContent = !touchPrompts ? 'Switch to mobile controls to edit the layout.' : started ? '' : 'Start a match to edit the layout on screen.';
 }
 byId('touch-edit-layout').onclick = () => {
  const { started, paused, touchPrompts } = state();
  if (!started || !touchPrompts) return;
