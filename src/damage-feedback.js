 ...
 return {
  ...
  update(sim,view){
   ...
   state.items.forEach(item=>{
    ...
    node.style.transform=`translate(-50%,-50%) rotate(${item.tilt}deg) scale(${damageFeedbackScale(sim.time-item.bumped)})`;
    node.style.left=Math.max(48,Math.min(width-48,point.x+item.side*offset[0]))+'px';
    node.style.top=Math.max(100,Math.min(height-35,point.y+offset[1]-Math.min(sim.time-item.bumped,2)*12))+'px';
   });
  }
 };
}
