...
// `reach` (the victim's radius or half size) and `radius` (the blast's): a
// victim already out of reach on the flat is out of reach in 3D too (never
// shorter), so it skips the sight walk: every blast asks this of every prop
// on the map. Infinity: out of reach.
export function blastReach(sim, x, z, victim, flatDistance, reach = 0, radius = Infinity) {
