...
export function createAbilityHUD() {
 ...
 return { update(sim) {
  ...
  updatePrimaryCooldown(sim.weapon==='shotgun'?{remaining:sim.shotgun.stored?sim.shotgun.hold:0,duration:SHOTGUN.holdTime,binding:'SHIFT / RMB',label:sim.shotgun.stored?'Stored charge expires':'Hold LMB / SPACE to charge'}:{remaining,duration:cooldown,binding:rifle?'E':'X',label:hexHint});
  ...
  const rangeHint=sim.weapon==='shotgun'?`${Math.round(range*100)}% charge · ${sim.shotgun.stored?sim.shotgun.hold.toFixed(1)+'s stored':'hold LMB / SPACE; SHIFT / RMB to store'}`:`${Math.round(range*100)}% to boundary · ${Math.max(0,(RULES.hexRange-(expanding?.age||0)*RULES.hexSpeed)/RULES.hexSpeed).toFixed(1)}s until expiry`;
