...
import { ArcBatch, LINE_POINTS, FORKS } from './arc-batch.js';
...
const SHARED = new Set([PULSE_SHAPE, ...Object.values(PULSE_RINGS)]);
...
export class ElectricEffects {
  ...
  pulse(n, radius, life = .55, scatter = false) {
    const mesh = new THREE.Mesh(PULSE_SHAPE, new THREE.MeshBasicMaterial({ color: '#d4f4ff', wireframe: true, transparent: true, depthWrite: false, toneMapped: false, blending: THREE.AdditiveBlending }));
    mesh.position.set(n.x, .72, n.z); this.scene.add(mesh);
    const core = new THREE.Mesh(PULSE_SHAPE, new THREE.MeshBasicMaterial({ color: '#e9fbff', transparent: true, opacity: .8, blending: THREE.AdditiveBlending, depthWrite: false, toneMapped: false }));
    core.position.copy(mesh.position); core.scale.setScalar(.06); this.scene.add(core);
    const rings = [0, 1].slice(0, this.detail.rings).map(i => {
      const ring = new THREE.Mesh(PULSE_RINGS[this.quality ? 64 : 40], new THREE.MeshBasicMaterial({ color: i ? '#72ceff' : '#eaffff', transparent: true, blending: THREE.AdditiveBlending, depthWrite: false, toneMapped: false }));
  ...
  dispose(e) {
    // Arcs own nothing to free. Pulses own their materials; their geometry is
    // shared by every pulse and stays.
    if (e.arc) return;
    for (const mesh of [e.mesh, e.core, ...(e.rings || [])].filter(Boolean)) {
      mesh.removeFromParent(); mesh.material.dispose();
      if (!SHARED.has(mesh.geometry)) mesh.geometry.dispose();
    }
  }
  // Every live pulse part, for the renderer's interior concealment pass.
  pulseParts() { const parts = []; for (const e of this.effects) if (!e.arc) parts.push(e.mesh, e.core, ...(e.rings || [])); return parts; }
