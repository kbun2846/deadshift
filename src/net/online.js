...
export async function goOnline({ role, code, map, local, createSim, name, server, config = NETWORK }) {
 ...
 if (role === 'host') return new HostSession({ transport: await hostRoom(code, { config, server }), map, local, createSim, config, name });
