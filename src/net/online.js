...
export async function goOnline({ role, code, map, local, createSim, name, password = '', server, config = NETWORK }) {
 ...
 if (role === 'host') return new HostSession({ transport: await hostRoom(code, { config, server }), map, local, createSim, config, name, password });
 return new ClientSession({ transport: await joinRoom(code, { config, server }), map, local, createSim, config, name, password });
}
