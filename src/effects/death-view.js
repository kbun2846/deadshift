...
// Seconds the camera takes to zoom onto the body before it holds still.
export const DEATH_ZOOM=2.2;
...
export class DeathView{
 ...
 start(event){
  ...
  // Dropped, it reads as dropped: a little darker than a weapon in hand.
  this.releaseGun=darkenWeapon(this.gun);
  ...
  // (Hills: the pool lies on the slope.)
  if(hilly(view))view.tiltToGround(this.pool,event.x,event.z,this.pool.rotation.y);
  ...
  // The pool's pieces (core, lobes, streaks) share one material: one draw.
  this.pool.updateMatrixWorld(true);
  const poolInverse=this.pool.matrixWorld.clone().invert(),parts=[...this.pool.children].map(m=>m.geometry.clone().applyMatrix4(new THREE.Matrix4().multiplyMatrices(poolInverse,m.matrixWorld)));
  this.pool.clear();this.poolMerged=mergeGeometries(parts);parts.forEach(g=>g.dispose());this.pool.add(new THREE.Mesh(this.poolMerged,this.material));
  const count={potato:16,performance:28,balanced:80,quality:120,extreme:160}[view.qualityName]||80;
 ...
 // death screen comes in (it sits on the right, or at the bottom when the
 // screen is narrow), the frame slides so the body and gore sit in the open
 // half instead of under it.
 cameraFrame(aspect=16/9,aside=true){
  const t=Math.min(1,this.age/DEATH_ZOOM),blend=t*t*(3-2*t),e=this.event,start=this.cameraStart;
  const k=aside?Math.min(1,Math.max(0,(this.age-DEATH_ZOOM+.4)/.6)):0,slide=k*k*(3-2*k);
  const height=start.height*(1-.44*blend),wide=aspect>1&&(typeof innerWidth!=='number'||innerWidth>700); // matches the CSS layout switch
  return {x:start.x+(e.x+(e.directionX||0)*.65-start.x)*blend+(wide?height*.3*slide:0),z:start.z+(e.z+(e.directionZ||0)*.65-start.z)*blend+(wide?0:height*.22*slide),height};
 }
 update(dt){
  ...
  // All landed: bones and organs become two draws instead of one each.
  if(t>2.4&&!this.bonesSettled){this.bonesSettled=true;this.placeBones(3);compact(this.bones);}
  if(!this.bonesSettled)this.placeBones(t);
 }
 placeBones(t){
  const e=this.event;
 ...
 clear(){
  ...
  this.gore?.dispose();this.gore=null;this.releaseGun?.();this.releaseGun=null;
  disposeMerged(this.bones);this.bonesSettled=false;
  ...
  for(const resource of [this.material,this.poolGeometry,this.lobeGeometry,this.dropGeometry,this.poolMerged])resource?.dispose();
  this.material=this.poolGeometry=this.lobeGeometry=this.dropGeometry=this.poolMerged=null;
