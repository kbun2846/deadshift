...
//
// s5-interiors: a house's `variant` is what its room was and what was left in
// it (world/colonial-interiors.js HOUSES): 'laying-out', 'weaver', 'kitchen',
// 'nursery', 'parlour', 'farm'. A chimney listed `over: <piece kind>` has no
// `at` of its own: it stands over that piece's flue wherever the room's
// layout put it (flueOf), so the stack outside is over the fire inside.
// `shutDoors` ([{ side, offset, width }]): a door drawn shut on the wall,
// no opening (the farm's back door, barred from within).
import { flueOf } from '../world/colonial-interiors.js';
...
export const COLONIAL = {
  ...
  // Weathered shingles, a tone per building (map-ui audit: two near-identical
  // dark greys, darker than the ground, made the town one slab): dark and
  // older, grey, silvered, brown-grey, mossy (damp, by trees or water), the
  // one newer cedar roof, and the smiths' soot.
  shingle: '#4d4842', shingle2: '#5a554e', silver: '#6f6b64', brownGrey: '#5d544a', moss: '#545440', cedar: '#6b4f3c', soot: '#403b36',
  brick: '#6e4436', stone: '#7f7b72',
...
const SPECS = {
  ...
  saltbox: { height: 2.9, color: COLONIAL.oxblood, roofColor: COLONIAL.silver, interiorStyle: 'colonial-house', variant: 'kitchen',
  ...
  cape: { height: 2.9, color: COLONIAL.clapboard, roofColor: COLONIAL.moss, interiorStyle: 'colonial-house', variant: 'weaver',
  ...
  // (Its front stoop runs on 1.1 m east: the rocking chair sits on it.)
  gambrel: { height: 2.6, color: '#76736b', roofColor: COLONIAL.shingle2, interiorStyle: 'colonial-house', variant: 'parlour', stoopRun: { front: 1.1 },
  ...
  'lit-cape': { height: 2.8, color: COLONIAL.clapboard, roofColor: COLONIAL.shingle, interiorStyle: 'colonial-lit-house', variant: 'laying-out',
  ...
  'saltbox-2': { height: 2.9, color: COLONIAL.clapboard, roofColor: COLONIAL.cedar, interiorStyle: 'colonial-house', variant: 'nursery',
  ...
  forge: { height: 2.5, color: '#5d554c', roofColor: COLONIAL.soot, doorWidth: 3.3,
  ...
  barn: { height: 3.6, color: COLONIAL.barnRed, roofColor: COLONIAL.silver, interiorStyle: 'barn', finish: 'boards', doorWidth: 3.2,
  ...
  mill: { height: 4, color: COLONIAL.clapboard, roofColor: COLONIAL.moss, interiorStyle: 'gristmill', doorWidth: 2.2, stone: 1.8,
  ...
  meetinghouse: { height: 4.2, color: COLONIAL.bone, roofColor: COLONIAL.boneRoof, trim: '#b9b2a0', interiorStyle: 'meetinghouse',
    ...
    windows: [w('front', -3.3, 1.1), w('front', 3.3, 1.1), w('front', -5.4, 1.1), w('front', 5.4, 1.1),
      // (The pulpit window, behind the pulpit, stands shuttered: s5-interiors.)
      w('back', 0, 1.1, { boarded: true }), w('back', -3.4, 1.1), w('back', 3.4, 1.1, { boarded: true }), w('back', -5.4, 1.1), w('back', 5.4, 1.1),
  ...
  // Fieldstone to the eave all round (stage 4 audit: it was clad in clapboard).
  tomb: { height: 2.2, stone: 2.2, color: '#6a665c', roofColor: COLONIAL.turf, interiorStyle: 'tomb', doorWidth: 1.4,
  ...
  farm: { height: 2.8, color: COLONIAL.clapboard, roofColor: COLONIAL.brownGrey, interiorStyle: 'colonial-house', variant: 'farm',
    roof: { kind: 'gable', axis: 'x', rise: 2.7 }, doors: ['front', 'right'], chimneys: [{ at: -.4, kind: 'stone', w: 1.4, d: 1.1 }],
    shutDoors: [{ side: 'back', offset: 0, width: 1 }],
    windows: [w('front', -2.6), w('front', 2.6), w('back', -2), w('back', 2, 1, { boarded: true }), w('left', 1.4)] },
  woodshed: { height: 2.2, color: COLONIAL.grey, roofColor: COLONIAL.silver, interiorStyle: 'shed', doorWidth: 3,
...
function stackOver(b, c) {
  ...
  const at = alongX ? flue.x : -flue.z, across = alongX ? flue.z : flue.x;
  const round = v => Math.round(v * 1000) / 1000;
  return { ...c, at: round(Math.max(-half, Math.min(half, at))), across: round(Math.max(-side, Math.min(side, across))) };
}
